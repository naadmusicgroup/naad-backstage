create index if not exists idx_earnings_admin_ledger_sort
on public.earnings (sale_date desc nulls last, created_at desc, id asc);
...
as $$
  ...
  filtered_earnings as materialized (
    ...
    where (target_artist_id is null or earning.artist_id = target_artist_id)
      ...
      and (target_period_month is null or upload.period_month = target_period_month)
  ...
  period_options as (
    select
      coalesce(jsonb_agg(
        jsonb_build_object(
          'value', period_month,
          'label', to_char(period_month, 'FMMonth YYYY'),
          'meta', null,
          'logoKey', null
        )
