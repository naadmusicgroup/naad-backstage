...
as $$
  ...
  select
    filtered.month,
    filtered.channel_id,
    nullif(filtered.normalized_territory, '__none__') as territory,
    coalesce(sum(filtered.streams), 0)::bigint as streams
  from filtered
  where (
    coalesce(target_territory, 'all') = 'all'
    or filtered.normalized_territory = target_territory
  )
  group by filtered.month, filtered.channel_id, filtered.normalized_territory
  order by filtered.month asc, filtered.channel_id asc nulls first, filtered.normalized_territory asc;
$$;
