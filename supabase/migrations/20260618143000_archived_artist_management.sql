...
as $$
  ...
  summary_counts as (
    select
      ...
      (select count(*) from public.artists where is_active = false) as archived_artist_count,
  ...
  select jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'archivedArtistCount', summary_counts.archived_artist_count,
    ...
    coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'email', artist.email,
          'artistSharePct', case
            when artist.artist_share_pct is null then null
            else to_char(artist.artist_share_pct, 'FM999999999999999999999999990.00')
          end,
          'avatarMode', coalesce(artist.avatar_mode, 'mesh'),
          'avatarPreset', coalesce(artist.avatar_preset, 'aurora'),
          'avatarCustomColors', artist.avatar_custom_colors,
          ...
          'dspProfiles', coalesce((
            select jsonb_agg(
              jsonb_build_object(
                ...
                'profileExists', dsp.profile_exists,
                'profileUrl', dsp.profile_url,
                'displayName', dsp.display_name,
                'avatarUrl', dsp.avatar_url,
                'updatedAt', dsp.updated_at
              )
              order by dsp.platform asc
            )
            from public.artist_dsp_profile_preferences as dsp
            where dsp.artist_id = artist.id
          ), '[]'::jsonb),
          'socialLinks', coalesce((
            select jsonb_agg(
              ...
              order by social.platform asc
            )
            from public.artist_social_links as social
            where social.artist_id = artist.id
          ), '[]'::jsonb)
        )
