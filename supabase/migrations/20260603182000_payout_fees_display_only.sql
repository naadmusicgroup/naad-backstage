...
create temporary table if not exists service_fee_cleanup_artists (
  artist_id uuid primary key
) on commit drop;

truncate table service_fee_cleanup_artists;

insert into service_fee_cleanup_artists (artist_id)
select distinct ledger.artist_id
...
set
  service_charge = service_fee_ledger.fee_amount,
  updated_at = now()
from service_fee_ledger
where request.id = service_fee_ledger.request_id
  and coalesce(request.service_charge, 0) = 0;

with service_fee_ledger as materialized (
  select
    ledger.id as ledger_id,
    ledger.reference_id as due_id
  from public.transaction_ledger as ledger
...
service_fee_due_ids as materialized (
  select distinct due_id
  from service_fee_ledger
  where due_id is not null
),
related_service_fee_ledger as materialized (
  select ledger.id as ledger_id
  from public.transaction_ledger as ledger
  left join service_fee_due_ids
    on service_fee_due_ids.due_id = ledger.reference_id
  where ledger.type = 'due_charge'
    and (
      ledger.id in (select ledger_id from service_fee_ledger)
      or service_fee_due_ids.due_id is not null
    )
),
cleared_dues as (
  update public.dues as due
  set ledger_entry_id = null
  from service_fee_due_ids
  where due.id = service_fee_due_ids.due_id
  returning due.id
),
deleted_ledger as (
  delete from public.transaction_ledger as ledger
  using related_service_fee_ledger
  where ledger.id = related_service_fee_ledger.ledger_id
  returning ledger.id
)
delete from public.dues as due
using service_fee_due_ids
where due.id = service_fee_due_ids.due_id;
...
begin
  for affected_artist_id in
    select artist_id from service_fee_cleanup_artists
  loop
