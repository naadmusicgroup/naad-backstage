create index if not exists idx_transaction_ledger_admin_payout_service_charge_marker
on public.transaction_ledger (idempotency_key)
where type = 'due_charge'
  and idempotency_key like 'admin_payout_service_charge:%';
...
revoke all on function public.update_admin_payout_financials(uuid, uuid, numeric, numeric)
  from public, anon, authenticated;
grant execute on function public.update_admin_payout_financials(uuid, uuid, numeric, numeric)
  to service_role;
...
begin
  ...
  if found then
    update public.dues
    set ledger_entry_id = null
    where id = service_charge_ledger_row.reference_id;
  ...
  return jsonb_build_object(
    ...
    'serviceCharge', '0',
    'serviceChargeDueId', null,
    'serviceChargeLedgerEntryId', null,
    'resultingBalance', resulting_balance::text
  );
end;
$$;
