alter table public.payout_requests
  add column if not exists bank_charge_pct numeric(5,2) default 1.00;

update public.payout_requests
set bank_charge_pct = 1.00
where bank_charge_pct is null;

alter table public.payout_requests
  alter column bank_charge_pct set default 1.00,
  alter column bank_charge_pct set not null;

alter table public.payout_requests
  drop constraint if exists payout_requests_bank_charge_pct_range;

alter table public.payout_requests
  add constraint payout_requests_bank_charge_pct_range
  check (bank_charge_pct >= 0 and bank_charge_pct <= 100);

drop function if exists public.update_admin_payout_financials(uuid, uuid, numeric, numeric);
drop function if exists public.update_admin_payout_financials(uuid, uuid, numeric, numeric, numeric);
...
declare
  ...
  service_charge_key text;
  service_charge_title text;
  charge_effective_at timestamptz;
  is_manual_payout boolean := false;
...
begin
  ...
  is_manual_payout := payout_ledger_row.idempotency_key = format('admin_manual_payout:%s', target_request_id);
  service_charge_key := case
    when is_manual_payout then format('admin_manual_payout_service_charge:%s', target_request_id)
    else format('admin_payout_service_charge:%s', target_request_id)
  end;
  service_charge_title := case
    when is_manual_payout then 'Manual payout service charge'
    else 'Payout service charge'
  end;
  charge_effective_at := coalesce(payout_row.paid_at, payout_row.reviewed_at, payout_row.created_at, now());
  ...
  where type = 'due_charge'
    and idempotency_key in (
      format('admin_manual_payout_service_charge:%s', target_request_id),
      format('admin_payout_service_charge:%s', target_request_id)
    )
  for update;

  if payout_row.status = 'rejected' and service_charge_amount > 0 then
    raise exception 'Rejected payout requests cannot carry a service charge.';
  end if;
  ...
  if service_charge_amount > 0 then
    if service_charge_ledger_row.id is null then
      ...
      values (
        ...
        format('%s for payout %s', service_charge_title, payout_row.id),
        service_charge_key,
        charge_effective_at,
        now()
      )
      ...
      set
        ...
        status = 'paid',
        due_date = charge_effective_at::date,
        paid_at = charge_effective_at,
        accepted_at = coalesce(accepted_at, charge_effective_at),
        accepted_by = coalesce(accepted_by, actor_admin_id),
        cancelled_at = null,
        cancelled_by = null,
        updated_at = now()
      ...
      if service_charge_due_row.id is null then
        insert into public.dues (
          artist_id,
          title,
          amount,
      ...
      update public.transaction_ledger
      set
        reference_id = service_charge_due_row.id,
        amount = service_charge_amount * -1,
        description = format('%s for payout %s', service_charge_title, payout_row.id),
        idempotency_key = service_charge_key,
        updated_at = now()
    ...
    update public.dues
    set ledger_entry_id = service_charge_ledger_row.id
    where id = service_charge_due_row.id
    returning *
    into service_charge_due_row;

    service_charge_due_id := service_charge_due_row.id;
    service_charge_ledger_entry_id := service_charge_ledger_row.id;
  elsif service_charge_ledger_row.id is not null then
    service_charge_due_id := service_charge_ledger_row.reference_id;
    service_charge_ledger_entry_id := service_charge_ledger_row.id;
    ...
    service_charge_due_id := null;
    service_charge_ledger_entry_id := null;
  end if;
