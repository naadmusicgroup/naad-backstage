...
import type { AdminNotificationRecord, AdminNotificationsResponse } from "~~/types/dashboard"
...
const { setUnreadNotificationCount } = useAdminNotificationState()
const page = ref(1)
...
const { data, error, pending, refresh } = useLazyFetch<AdminNotificationsResponse>("/api/admin/notifications", {
  query: notificationQuery,
})

const response = computed<AdminNotificationsResponse>(() => (
  data.value ?? {
    notifications: [],
    unreadCount: 0,
    totalCount: 0,
...
function notificationTypeTone(notification: AdminNotificationRecord) {
  switch (notification.type) {
    case "payout_requested":
    case "payout_details_changed":
      return "warning"
    case "release_change_requested":
    case "release_submitted":
    case "publishing_submitted":
      return "info"
  }
}
...
// Viewing the queue clears it for every admin (read state is shared).
watch(
  () => [pending.value, unreadCount.value, data.value],
  () => {
    if (!pending.value && data.value && unreadCount.value) {
...
<template>
  <div class="page">
    <DataPanel title="Notifications" title-level="h1">
      ...
      <DashboardSkeleton v-else-if="pending && !data" layout="admin-notifications" :rows="5" />
      ...
      <template v-else>
        ...
        <AppEmptyState
          ...
          description="Release submissions, catalog requests, payout requests, and publishing registrations will appear here once artists act."
        />
          ...
          <div class="notification-timeline">
            ...
            >
              ...
              >
                ...
                <p v-if="notification.artistName" class="detail-copy notification-artist">
...
.notification-artist {
  margin: 0;
  font-weight: 600;
  color: color-mix(in srgb, var(--priority) 70%, var(--muted-foreground));
}
