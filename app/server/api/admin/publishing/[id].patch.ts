...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<AdminPublishingUpdateInput>(event)
  ...
  const { data, error } = await supabase.rpc("update_publishing_earning", {
  ...
  if (error || !data) {
    throw createError({
      ...
      statusMessage: error?.message || "Unable to update this publishing credit.",
    })
  }

  const result = data as AdminPublishingMutationResponse

  await logAdminActivity(supabase, profile.id, "publishing.updated", "publishing_earning", entryId, {
