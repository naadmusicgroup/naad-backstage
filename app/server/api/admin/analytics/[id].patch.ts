...
import type { AdminAnalyticsMutationResponse, AdminAnalyticsUpdateInput } from "~~/types/admin"
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<AdminAnalyticsUpdateInput>(event)
  ...
  await assertReleaseBelongsToArtist(supabase, existing.artist_id, releaseId)
  ...
  if (error || !data) {
    throw createError({
      ...
      statusMessage: error?.code === "23505"
        ...
        : error?.message || "Unable to update this analytics snapshot.",
    })
  }

  await logAdminActivity(supabase, profile.id, "analytics.updated", "analytics_snapshot", entryId, {
    release_id: releaseId,
    platform,
    metric_type: metricType,
    value,
